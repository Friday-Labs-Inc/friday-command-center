...
RECORDED_KINDS = ("env", "gps", "odom", "imu", "map", "voxel", "mission", "terrain", "terrain_grid", "terrain_keyframe", "autonomy")
...
def _event(topic: str, payload: bytes):
    ...
    kind = ("odom" if "/tlm/odom" in topic else "fault" if "/tlm/fault" in topic
            ...
            else "voxel" if "/tlm/voxel" in topic
...
async def proxy_mission_dispatch(req: Request):
    return await _proxy(req, "/api/mission/dispatch")
